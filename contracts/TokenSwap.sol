...
contract TokenSwap {
    ...
    function addLiquidity(
        uint _token1Amount,
        uint _token2Amount
    ) public returns (uint liquidityAdded) {
        require(
    ...
    function removeLiquidity(
        uint _amount
    ) public returns (uint token1Amount, uint token2Amount) {
        require(
    ...
    function swap(address _fromToken, address _toToken, uint _amount) public {
        require(
            _fromToken == token1 || _fromToken == token2,
            "Invalid from token address"
        );
        require(
            _toToken == token1 || _toToken == token2,
    ...
    function calculateSwapAmount(
        ...
        address _toToken,
        uint _amount
    ) private view returns (uint toTokenAmount) {
    ...
    function getBalances()
        public
        view
        returns (uint token1Balance, uint token2Balance)
    {
