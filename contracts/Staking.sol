...
contract Staker {
    ...
    modifier stakeNotCompleted() {
        bool completed = stakeExecContract.completed();
     ...
     * @param stakeExecContractAddress Address of the external contract that will hold stacked funds
     */
    ...
    function execute() public stakeNotCompleted deadlineReached(true) {
        ...
        // (bool sent, bytes memory data) = stakeExecContract.complete{value: contractBalance}();
        (bool sent, ) = address(stakeExecContract).call{value: contractBalance}(
            abi.encodeWithSignature("complete()")
        );
        require(sent, "stakeExecContract.complete failed");
    }
